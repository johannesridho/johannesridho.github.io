...
type Props = {
  tags: string[]
  data: CollectionEntry<"achievements">[]
}

export default function Achievements({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [achievements, setAchievements] = createSignal<CollectionEntry<"achievements">[]>([])

  createEffect(() => {
    setAchievements(data.filter((entry) => 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
      <div class="col-span-3 sm:col-span-1">
        <div class="sticky top-24">
          <div class="text-sm font-semibold mb-2 text-black dark:text-white">Filter</div>
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm mb-2">
            Showing {achievements().length} of {data.length} achievements
          </div>
          <ul class="flex flex-col gap-3">
            {achievements().map((achievement) => (
              <li>
                <ArrowCard entry={achievement} />
              </li>
            ))}
          </ul>
        </div>
